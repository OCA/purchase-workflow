...
class ComputedPurchaseOrderLine(models.Model):
    ...
    computed_purchase_order_id = fields.Many2one(
        "computed.purchase.order", "Order Reference", required=True, ondelete="cascade"
    )
    ...
    product_id = fields.Many2one(
        "product.product", "Product", required=True, domain=[("purchase_ok", "=", True)]
    )
    ...
    product_price = fields.Float(
        "Supplier Product Price", digits=dp.get_precision("Product Price")
    )
    ...
    product_price_inv = fields.Float(
        ...
        string="Supplier Product Price",
        multi="product_code_name_price",
    )
    price_policy = fields.Selection(
        [("uom", "per UOM"), ("package", "per Package")], default="uom", required=True
    )
    ...
    subtotal = fields.Float(
        compute="_compute_subtotal_price", digits=dp.get_precision("Product Price")
    )
    package_qty = fields.Float("Package quantity")
    package_qty_inv = fields.Float(
        compute="_compute_product_information",
        inverse="_inverse_package_qty",
    ...
    displayed_average_consumption = fields.Float("Average Consumption", digits=(12, 3))
    ...
    psi_id = fields.Many2one(comodel_name="product.supplierinfo", ondelete="set null")
    shelf_life = fields.Integer(
        string="Shelf life (days)", compute="compute_shelf_life", store=True
    )
    ...
    def onchange_purchase_qty(self):
        for cpol in self:
            if cpol.package_qty_inv:
                cpol.purchase_qty_package = cpol.purchase_qty / cpol.package_qty_inv
    ...
    def _compute_purchase_qty(self):
        for cpol in self:
            if cpol.purchase_qty_package == int(cpol.purchase_qty_package):
                cpol.purchase_qty = cpol.package_qty_inv * cpol.purchase_qty_package
                psi = cpol.get_psi()
                cpol.psi_id = psi
        ...
        for line in self:
            if line.price_policy == "package":
                if line.package_qty_inv:
                    line.product_price_inv_eq = (
                        line.product_price_inv / line.package_qty_inv
                    )
                else:
    ...
    def _compute_subtotal_price(self):
        for line in self:
            net_unit_price = line.product_price_inv * (1 - line.discount_inv / 100.0)
            if line.price_policy == "package":
                line.subtotal = (
        ...
        if self.product_id:
            ...
            if self.computed_purchase_order_id:
                ...
                if self.product_id.id in products:
                    raise ValidationError(_("This product is already in the list!"))
