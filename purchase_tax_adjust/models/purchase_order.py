            ...
            for amount_by_group_list in tax_totals["groups_by_subtotal"].values():
                for amount_by_group in amount_by_group_list:
                    tax_lines = order.order_line.filtered(
                        ...
                        in line.taxes_id.mapped("tax_group_id").ids
                    )
    ...
    def _compute_amount(self):
        ...
        for line in self:
            ...
            if line.tax_adjust and line.price_tax and line.price_tax != price_tax:
                ...
                continue
            res = super(PurchaseOrderLine, line)._compute_amount()
        return res

    @api.onchange("product_qty", "price_unit", "price_subtotal", "taxes_id")
    def _onchange_line_without_tax(self):
        """Auto refresh tax. if line purchase is change"""
        self._origin.order_id.action_refresh_taxes()
