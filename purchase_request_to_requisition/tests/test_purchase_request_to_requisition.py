...
class TestPurchaseRequestToRequisition(common.TransactionCase):
    ...
    def test_purchase_request_to_purchase_requisition(self):
        ...
        self.assertEquals(
            purchase_request.state, requisition_id.state, "Should have the same state"
        )
        requisition_id.tender_in_progress()
        requisition_id.tender_open()
        vals = {"partner_ids": [(4, self.env.ref("base.res_partner_12").id, 0)]}
        requisition_partner_id = self.purchase_requisition_partner_model.with_context(
        ...
        self.assertEquals(
            len(purchase_id.order_line.purchase_request_lines),
            1,
