...
class PurchaseOrder(models.Model):
    ...
    def onchange_amount_untaxed(self):
        approval_block_id = self.env["purchase.approval.block.reason"].search(
            [("amount", "<=", self.amount_untaxed)], limit=1, order="amount desc"
        )
    ...
    def create(self, vals_list):
        po = super().create(vals_list)
        for rec in po:
            if rec.approval_block_id:
                po.message_post(
                    body=_('Order "%(order)s" blocked with reason "%(reason)s"')
                    % {"order": rec.name, "reason": rec.approval_block_id.name}
                )
        return po
    ...
    def write(self, vals):
        ...
        for po in self:
            if "approval_block_id" in vals and vals["approval_block_id"]:
                po.message_post(
                    body=_('Order "%(order)s" blocked with reason "%(reason)s"')
                    % {"order": po.name, "reason": po.approval_block_id.name}
                )
            elif "approval_block_id" in vals and not vals["approval_block_id"]:
                po.message_post(
                    body=_('Order "%(order)s" approval block released.')
                    % {"order": po.name}
                )
        return res
