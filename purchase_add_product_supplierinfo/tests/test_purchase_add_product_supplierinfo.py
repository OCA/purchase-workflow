...
class TestPurchaseAddProductSupplierinfo(TransactionCase):
    def test_add_product_supplierinfo_on_product_template(self):
        ...
        result8 = purchase_8.with_context(
            wizard_supplierinfo_confirm=True
        ).button_confirm()
        # open new form when products to update
        self.assertEquals(result8["view_mode"], "form")
        self.assertEquals(
            result8["res_model"], "purchase.add.product.supplierinfo"
        )
        self.assertEquals(
        ...
        self.assertNotEquals(len(supplierinfo_ids), 0)
    ...
    def test_add_product_supplierinfo_on_product_product(self):
        ...
        result9 = purchase_9.with_context(
            wizard_supplierinfo_confirm=True
        ).button_confirm()
        # open new form when products to update
        self.assertEquals(result9["view_mode"], "form")
        self.assertEquals(
            result9["res_model"], "purchase.add.product.supplierinfo"
        )
        self.assertEquals(
    ...
    def test_without_add_product_supplierinfo(self):
        ...
        result11 = purchase_11.with_context(
            wizard_supplierinfo_confirm=True
        ).button_confirm()
        self.assertEquals(result11, True)
