...
class purchase_order(osv.osv):
    ...
    _columns = {
        ...
        'bid_partial': fields.boolean('Bid partially selected', readonly=True, help="True if the bid has been partially selected"),
        'consignee_id': fields.many2one('res.partner', 'Consignee', help="Person responsible of delivery"),
        'tender_bid_receipt_mode': fields.function(lambda self,*args,**kwargs:self._get_tender(*args,**kwargs), multi="callforbids", readonly=True, 
                    type='selection', selection=[('open','Open'),('sealed','Sealed')],
    ...
    def _get_tender(self, cr, uid, ids, fields, args, context=None):
        orders=self.read(cr, uid, ids, ['requisition_id'], context=context, load='_classic_write')
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if context.get('draft_po'):
            ...
            wf_service.trg_validate(uid, 'purchase.order', newly_id, 'draft_po', cr)
        return newly_id
