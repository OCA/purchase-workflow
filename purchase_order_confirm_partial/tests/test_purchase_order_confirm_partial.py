# Copyright (C) 2024 Cetmix OÜ
# License AGPL-3.0 or later (http://www.gnu.org/licenses/agpl).
from odoo import exceptions
from odoo.tests import TransactionCase, tagged


@tagged("post_install", "-at_install")
class TestPurchaseOrderConfirmPartial(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.env["ir.config_parameter"].sudo().set_param(
            "purchase_order_confirm_partial.enabled", True
        )
        cls.purchase_order = cls.env["purchase.order"].create(
            {
                "name": "PO-TEST",
                "partner_id": cls.env.ref("base.res_partner_1").id,
            }
        )
        cls.po_line_1 = cls.env["purchase.order.line"].create(
            {
                "order_id": cls.purchase_order.id,
                "product_id": cls.env.ref("product.product_product_6").id,
                "product_qty": 10,
                "price_unit": 100,
            }
        )
        cls.po_line_2 = cls.env["purchase.order.line"].create(
            {
                "order_id": cls.purchase_order.id,
                "product_id": cls.env.ref("product.product_product_7").id,
                "product_qty": 7,
                "price_unit": 200,
            }
        )

    def test_confirmation_with_save_unconfirmed_all(self):
        self.env["ir.config_parameter"].sudo().set_param(
            "purchase_order_confirm_partial.save_unconfirmed", True
        )
        # Confirm PO partially
        self.env["purchase.order.confirm.partial"].create(
            {"purchase_order_id": self.purchase_order.id, "mode": "all"}
        ).action_confirm()

        # Check PO state
        self.assertEqual(
            self.purchase_order.state, "purchase", "Purchase order is not confirmed"
        )

        # Since we have confirmed all of the lines, there is no need to save
        # the unconfirmed part of PO
        unconfirmed_po = self.env["purchase.order"].search(
            [("name", "like", "PO-TEST-U")]
        )
        self.assertFalse(
            unconfirmed_po,
            "Unconfirmed part of PO has been created when all of lines were confirmed.",
        )

        # Check original order lines quantities
        # (they should stay the same)
        self.assertEqual(
            self.po_line_1.product_qty,
            10,
            "Original order line 1 qty is wrong",
        )
        self.assertEqual(
            self.po_line_2.product_qty,
            7,
            "Original order line 2 qty is wrong",
        )

    def test_confirmation_with_save_unconfirmed_selected(self):
        self.env["ir.config_parameter"].sudo().set_param(
            "purchase_order_confirm_partial.save_unconfirmed", True
        )
        # Confirm PO partially
        self.env["purchase.order.confirm.partial"].create(
            {
                "purchase_order_id": self.purchase_order.id,
                "mode": "selected",
                "line_ids": [
                    (
                        0,
                        0,
                        {
                            "po_line_id": self.po_line_1.id,
                            "confirmed_qty": 4,
                        },
                    ),
                ],
            }
        ).action_confirm()

        # Check PO state
        self.assertEqual(
            self.purchase_order.state, "purchase", "Purchase order is not confirmed"
        )

        # Since we have confirmed selected line, we need to save
        # the unconfirmed part of PO
        unconfirmed_po = self.env["purchase.order"].search(
            [("name", "like", "PO-TEST-U")]
        )
        self.assertTrue(
            unconfirmed_po,
            "Unconfirmed part of PO hasn't been created when order was confirmed partially.",
        )
        # Check unconfirmed PO status
        self.assertEqual(
            unconfirmed_po.state, "cancel", "Unconfirmed PO is not in cancel state"
        )
        # Check unconfirmed order lines quantities
        # (they should be equal to statement original_qty - confirmed_qty)
        self.assertEqual(
            unconfirmed_po.order_line.filtered(
                lambda x: x.po_line_id == self.po_line_1
            ).product_qty,
            6,
            "Unconfirmed part of order line 1 qty is wrong",
        )
        self.assertEqual(
            unconfirmed_po.order_line.filtered(
                lambda x: x.po_line_id == self.po_line_2
            ).product_qty,
            7,
            "Unconfirmed part of order line 2 qty is wrong",
        )

        # Check original order lines quantities
        # (they should be reduced accordingly to the confirmed quantities)
        self.assertEqual(
            self.po_line_1.product_qty,
            4,
            "Original order line 1 qty is wrong",
        )
        self.assertEqual(
            self.po_line_2.product_qty,
            0,
            "Original order line 2 qty is wrong",
        )

    def test_confirmation_without_save_unconfirmed_selected(self):
        # Confirm PO partially
        self.env["purchase.order.confirm.partial"].create(
            {
                "purchase_order_id": self.purchase_order.id,
                "mode": "selected",
                "line_ids": [
                    (
                        0,
                        0,
                        {
                            "po_line_id": self.po_line_1.id,
                            "confirmed_qty": 4,
                        },
                    ),
                ],
            }
        ).action_confirm()

        # Check PO state
        self.assertEqual(
            self.purchase_order.state, "purchase", "Purchase order is not confirmed"
        )

        # Since we have confirmed selected line, we need to save
        # the unconfirmed part of PO (but it's disabled)
        unconfirmed_po = self.env["purchase.order"].search(
            [("name", "like", "PO-TEST-U")]
        )
        self.assertFalse(
            unconfirmed_po,
            "Unconfirmed part of PO has been created but settings option is disabled.",
        )

        # Check original order lines quantities
        # (they should be reduced accordingly to the confirmed quantities)
        self.assertEqual(
            self.po_line_1.product_qty,
            4,
            "Original order line 1 qty is wrong",
        )
        self.assertEqual(
            self.po_line_2.product_qty,
            0,
            "Original order line 2 qty is wrong",
        )

    def test_purchase_order_confirm_errors(self):
        # Try to create confirmation for quantity less the zero
        with self.assertRaises(exceptions.ValidationError):
            self.env["purchase.order.confirm.partial"].create(
                {
                    "purchase_order_id": self.purchase_order.id,
                    "mode": "selected",
                    "line_ids": [
                        (
                            0,
                            0,
                            {
                                "po_line_id": self.po_line_1.id,
                                "confirmed_qty": -4,
                            },
                        ),
                    ],
                }
            )
        # Try to create confirmation for quantity greater than original quantity
        with self.assertRaises(exceptions.ValidationError):
            self.env["purchase.order.confirm.partial"].create(
                {
                    "purchase_order_id": self.purchase_order.id,
                    "mode": "selected",
                    "line_ids": [
                        (
                            0,
                            0,
                            {
                                "po_line_id": self.po_line_1.id,
                                "confirmed_qty": 15,
                            },
                        ),
                    ],
                }
            )
