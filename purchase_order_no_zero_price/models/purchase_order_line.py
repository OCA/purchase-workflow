...
ENABLE_TESTING_CONTEXT_KEY = "testing_purchase_order_no_zero_price"
...
class PurchaseOrderLine(models.Model):
    ...
    def _check_price_unit_zero(self):
        # In tests, other modules do not know
        # that they can't create lines with price 0.
        # Only change the default behavior when explicitly testing this
        is_testing = getattr(threading.currentThread(), "testing", False)
        if not is_testing or (
            is_testing and self.env.context.get(ENABLE_TESTING_CONTEXT_KEY)
        ):
