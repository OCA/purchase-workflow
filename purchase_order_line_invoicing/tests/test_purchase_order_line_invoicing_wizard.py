...
class TestPurchaseOrderLineInvoiceWizard(TransactionCase):
    ...
    def setUp(self):
        super(TestPurchaseOrderLineInvoiceWizard, self).setUp()
    ...
    def test1(self):
        ...
        wizard = self.env['purchase.order.line.invoice.wizard'].with_context(
            active_ids=purchase_order_lines.ids).create({})
        ...
        self.assertEqual(invoice.invoice_line_ids.filtered(
            lambda x: x.purchase_line_id.id ==
            line1.purchase_order_line_id.id).quantity, 3)
        self.assertEqual(invoice.invoice_line_ids.filtered(
            lambda x: x.purchase_line_id.id ==
            line2.purchase_order_line_id.id).quantity, 4)
