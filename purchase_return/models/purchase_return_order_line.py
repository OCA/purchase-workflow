...
class PurchaseReturnOrderLine(models.Model):
    ...
    qty_delivered_method = fields.Selection(
        ...
        store=True,
        help="According to product configuration, the "
        "received quantity can be automatically computed by mechanism :\n"
    ...
    qty_delivered_manual = fields.Float(
        "Manual Delivered Qty", digits="Product Unit of Measure", copy=False
    )
    ...
    def _compute_qty_invoiced(self):
        for line in self:
            ...
            if line.order_id.state in ["purchase", "done"]:
                if line.product_id.purchase_method == "purchase" or line.refund_only:
    ...
    def _inverse_qty_delivered(self):
        ...
        modify manually (`qty_delivered_method` = 'manual' only),
        then we put the value in `qty_delivered_manual`. Otherwise,
        `qty_delivered_manual` should be False since the
    ...
    def _track_qty_delivered(self, new_qty):
        self.ensure_one()
        if new_qty != self.qty_delivered and self.order_id.state == "purchase":
            self.order_id.message_post_with_view(
                "purchase.track_po_line_qty_delivered_template",
