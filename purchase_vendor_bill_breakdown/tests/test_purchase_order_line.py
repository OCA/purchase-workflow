...
class TestPurchaseOrderLine(PurchaseTransactionCase):
    ...
    TEST 8 - Throws an error when Component is not unique or equal to parent product
    """
    ...
    def test_component_is_not_unique_or_parent_product(self):
        ...
        with Form(self.purchase_order_test_1) as form, form.order_line.new() as line:
            ...
            line.price_unit = 50.0
            line.product_qty = 5.0
        order_line = self.purchase_order_test_1.order_line
        with self.assertRaises(exceptions.ValidationError):
            with Form(
                order_line, view=self.component_view
            ) as form, form.component_ids.new() as line:
        ...
        with self.assertRaises(exceptions.ValidationError):
            with Form(
                ...
                line.component_id = self.product_product_test_1

        with self.assertRaises(exceptions.ValidationError):
            with Form(
                order_line, view=self.component_view
            ) as form, form.component_ids.edit(0) as line:
