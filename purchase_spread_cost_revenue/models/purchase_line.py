...
class PurchaseOrderLine(models.Model):
    ...
    def spread_details(self):
        """Button on the purchase lines tree view of the purchase order
        ...
        # In case no spread board is linked to the purchase line
        ...
        return {
            "name": _("Link Purchase Line with Spread Board"),
            "view_mode": "form",
            "res_model": "account.spread.purchase.line.link.wizard",
            "type": "ir.actions.act_window",
            "target": "new",
            "context": ctx,
        }
    ...
    def create_auto_spread(self):
        """Create auto spread table for each purchase line, when needed"""

        def _filter_line(aline, pline):
            """Find matching template auto line with purchase line"""
            if aline.product_id and pline.product_id != aline.product_id:
                return False
            return True
    ...
    def _prepare_account_move_line(self, move=False):
        ...
        # Creating invoice from purchase order, ensure same spread account
        if self.spread_id:
            res["spread_on_purchase"] = True
            res["spread_id"] = self.spread_id.id
            res["account_id"] = self.spread_id.credit_account_id.id
        return res
