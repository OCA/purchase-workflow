...
class TestPurchaseDepositCompanyAmount(TransactionCase):
    ...
    def _create_purchase_order(self, lines=None, currency=None, qty=1.0):
    ...
    def _register_deposit(self, po, amount=30, method="percentage"):
        ...
        with Form(wizard_env) as advance_form:
            advance_form.advance_payment_method = method
            advance_form.amount = amount
    ...
    def _post_deposit_bill(self, po, company_amount, amount=30, method="percentage"):
        deposit_bill = self._register_deposit(po, amount, method)
    ...
    def test_a_partial_bill_of_a_pinned_order_is_refused(self):
        # 100 units x USD 10, deposit of USD 300 settled for JPY 39000. The
        # whole deposit is relieved on the first bill, so a bill for 40 units
        # would take the entire JPY 9000 difference onto those units.
        po = self._create_purchase_order([(self.product, 10.0)], qty=100.0)
        self._post_deposit_bill(po, 39000)
        bill = self._create_final_bill(po)
        with Form(bill) as bill_form:
            with bill_form.invoice_line_ids.edit(0) as line_form:
                line_form.quantity = 40
        self.assertEqual(self._offset_line(bill).balance, -39000)
        self.assertEqual(self._goods_lines(bill).balance, 55000)
        with self.assertRaises(UserError):
            bill.action_post()
        # Neither a draft for the rest of the order nor posting that rest
        # first excuses it: the difference is shared by the goods of this
        # bill, whatever the other bills of the order hold.
        rest = self._create_final_bill(po, receive=False)
        self.assertEqual(self._goods_lines(rest).quantity, 60)
        self.assertFalse(self._offset_line(rest).quantity)
        with self.assertRaises(UserError):
            bill.action_post()
        rest.action_post()
        with self.assertRaises(UserError):
    ...
    def test_a_difference_the_goods_cannot_hold_is_not_forced(self):
        po = self._create_purchase_order()
        self._post_deposit_bill(po, 3900)
        bill = self._create_final_bill(po)
        goods_line = self._goods_lines(bill)
        self.assertEqual(goods_line._get_rate_based_balance(), 16000)
        self.assertEqual(
            bill._get_absorbed_targets(goods_line, -1000)[goods_line], 15000
        )
        # A difference the goods cannot hold would run the line against its
        # foreign currency amount, which the ledger forbids.
        self.assertFalse(bill._get_absorbed_targets(goods_line, -20000))

    def test_a_bill_smaller_than_the_difference_keeps_the_conversion(self):
        # 100 units x USD 10 with a JPY 9000 difference, first billed 2 units
        # (JPY 3200): absorbing the difference would take the goods line to
        # JPY -5800, against its USD 20.
        po = self._create_purchase_order([(self.product, 10.0)], qty=100.0)
        self._post_deposit_bill(po, 39000)
        bill = self._create_final_bill(po)
        with Form(bill) as bill_form:
        ...
        # Nothing is pinned and nothing leaks onto the payable: the draft is
        # left exactly as Odoo would convert it.
        self.assertEqual(self._goods_lines(bill).balance, 3200)
        self.assertEqual(self._offset_line(bill).balance, -48000)
        self.assertEqual(self._payable_line(bill).balance, 44800)
        with self.assertRaises(UserError):
            bill.action_post()
