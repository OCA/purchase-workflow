...
class PurchaseInvoicePlan(models.Model):
    ...
    allocation_method = fields.Selection(
        related="purchase_id.invoice_plan_method",
        string="Allocation Method",
    )
    allocation_ids = fields.One2many(
        comodel_name="purchase.invoice.plan.allocation",
    ...
    def _uses_line_allocation(self):
        self.ensure_one()
        return self.allocation_method in ("manual", "sequential")
    ...
    def _compute_amount(self):
        ...
        sequential = self.filtered(
            lambda plan: plan.allocation_method == "sequential" and not plan.invoiced
        )
        for plan in sequential:
            amount_untaxed = plan.purchase_id._origin.amount_untaxed
        ...
        # Base behavior for proportional/manual/invoiced plans.
        return super(PurchaseInvoicePlan, self - sequential)._compute_amount()

    def action_prepare_allocations(self):
        self.ensure_one()
        self.purchase_id._prepare_invoice_plan_allocations()
    ...
    def _validate_allocation(self, check_invoiceable=False):
        for plan in self.filtered(
            ...
            # per-line totals against ordered quantities on purchase.order.
            ...
            for allocation in allocations:
                purchase_line = allocation.purchase_line_id
                if (
                    float_compare(
                        allocation.quantity,
                        purchase_line.qty_to_invoice,
                        precision_rounding=purchase_line.product_uom.rounding,
                    )
                    == 1
                ):
                    raise ValidationError(
                        ...
                        % {
                            "line": purchase_line.name,
                            "planned": allocation.quantity,
                            "available": purchase_line.qty_to_invoice,
                        }
                    )
    ...
    def _get_plan_qty(self, order_line, percent):
        ...
        allocation = self.allocation_ids.filtered(
            lambda line: line.purchase_line_id == order_line
        )
...
class PurchaseInvoicePlanAllocation(models.Model):
    _name = "purchase.invoice.plan.allocation"
    _description = "Purchase Invoice Plan Line Allocation"
    _order = "plan_id, purchase_line_id"
    ...
    allocated_elsewhere_quantity = fields.Float(
        compute="_compute_remaining_quantity",
        string="Other Installments",
        digits="Product Unit of Measure",
    )
    ...
    currency_id = fields.Many2one(related="purchase_id.currency_id")
    amount = fields.Monetary(compute="_compute_amount", string="Untaxed Amount")
    no_edit = fields.Boolean(related="plan_id.no_edit")

    _sql_constraints = [
        (
            "unique_plan_purchase_line",
            "UNIQUE(plan_id, purchase_line_id)",
            "A purchase order line can only be allocated once per installment.",
        )
    ]

    @api.depends("quantity", "purchase_line_id.price_unit_discounted")
    def _compute_amount(self):
        for allocation in self:
            allocation.amount = allocation.currency_id.round(
                allocation.quantity * allocation.purchase_line_id.price_unit_discounted
            )

    @api.depends(
        "quantity",
        "purchase_line_id.product_qty",
        "purchase_line_id.order_id.invoice_plan_ids.allocation_ids.quantity",
    )
    def _compute_remaining_quantity(self):
        # Sum siblings once per order, from memory so unsaved inline edits count.
        totals = {}
        for sibling in self.purchase_id.invoice_plan_ids.allocation_ids:
            key = (sibling.purchase_id.id, sibling.purchase_line_id.id)
            totals[key] = totals.get(key, 0.0) + sibling.quantity
        for allocation in self:
            total = totals.get(
                (allocation.purchase_id.id, allocation.purchase_line_id.id), 0.0
            )
            allocation.allocated_elsewhere_quantity = total - allocation.quantity
            allocation.remaining_quantity = allocation.ordered_quantity - total

    @api.constrains("plan_id", "purchase_line_id")
    def _check_purchase_line_order(self):
        for allocation in self:
            if allocation.purchase_line_id.order_id != allocation.purchase_id:
                raise ValidationError(
                    self.env._(
                        "The allocated purchase order line must belong to the "
                        "invoice plan's purchase order."
                    )
                )
            if allocation.purchase_line_id.display_type:
                raise ValidationError(
                    self.env._(
                        "Sections and notes cannot be allocated to an invoice plan."
                    )
                )
            if not allocation.purchase_line_id.product_id:
                raise ValidationError(
                    self.env._("An allocated purchase order line must have a product.")
                )
    ...
    def create(self, vals_list):
        plans = self.env["purchase.invoice.plan"].browse(
