...
class PurchaseOrder(models.Model):
    ...
    def _get_sequential_num_installment(self):
        ...
        return sum(
            round(max(line.product_qty for line in group)) for group in groups.values()
        )

    def _validate_sequential_quantities(self):
        """Sequential allocation requires positive integer quantities."""
        self.ensure_one()
        purchase_lines = self._get_invoice_plan_allocatable_lines()
        if not purchase_lines:
            raise ValidationError(
                self.env._(
                    "The 'Sequential Grouped' method requires at least "
                    "one invoiceable purchase order line."
                )
            )
    ...
    def _prepare_invoice_plan_allocations(self):
        Allocation = self.env["purchase.invoice.plan.allocation"]
        for order in self.filtered(
            lambda purchase: purchase.invoice_plan_method in ("manual", "sequential")
        ):
            purchase_lines = order._get_invoice_plan_allocatable_lines()
            plans = order.invoice_plan_ids.filtered(
                lambda invoice_plan: not invoice_plan.invoiced
            )
            ...
            vals_list = [
                {"plan_id": plan.id, "purchase_line_id": purchase_line.id}
                for plan in plans
                for purchase_line in purchase_lines
                - plan.allocation_ids.purchase_line_id
            ]
            if vals_list:
    ...
    def _auto_allocate_sequential(self, plans):
        ...
        if not plans:
            return
        installment_plans = self.invoice_plan_ids
        ...
        for group_lines in self._get_sequential_line_groups().values():
            group_size = round(max(line.product_qty for line in group_lines))
            for installment_offset in range(group_size):
                ...
                if plan and plan.id in open_plan_ids:
                    for line in group_lines:
                        if installment_offset < round(line.product_qty):
                            vals_list.append(
                                {
                                    "plan_id": plan.id,
        ...
        if vals_list:
            self.env["purchase.invoice.plan.allocation"].create(vals_list)

    def _validate_invoice_plan_allocations(self, invoiceable_plan=False):
        for order in self.filtered(
            lambda purchase: purchase._uses_invoice_plan_allocation()
        ):
            ...
            for allocation in plans.allocation_ids:
                planned_by_line[allocation.purchase_line_id] += allocation.quantity
            for purchase_line in order._get_invoice_plan_allocatable_lines():
                planned_qty = planned_by_line[purchase_line]
                if (
                    float_compare(
                        planned_qty,
    ...
    def action_allocate_evenly(self):
        ...
        Allocation = self.env["purchase.invoice.plan.allocation"]
        for order in self.filtered(
            lambda purchase: purchase.invoice_plan_method == "manual"
        ):
            plans = order.invoice_plan_ids.filtered(
                lambda plan: not plan.invoiced
            ).sorted("installment")
            if not plans:
                continue
            count = len(plans)
            vals_list = []
            invoiced_plans = order.invoice_plan_ids.filtered("invoiced")
            invoiced_by_line = defaultdict(float)
            for allocation in invoiced_plans.allocation_ids:
                invoiced_by_line[allocation.purchase_line_id] += allocation.quantity
            for purchase_line in order._get_invoice_plan_allocatable_lines():
                rounding = purchase_line.product_uom.rounding
                invoiced_plan_qty = invoiced_by_line[purchase_line]
                quantity = purchase_line.product_qty - invoiced_plan_qty
                if float_compare(quantity, 0.0, precision_rounding=rounding) == -1:
                    raise ValidationError(
                        self.env._(
    ...
    def action_view_invoice_plan_allocations(self):
        ...
        return {
            ...
            "res_model": "purchase.invoice.plan.allocation",
            "view_mode": "list",
            "view_id": self.env.ref(
                "purchase_invoice_plan_allocation."
                "view_purchase_invoice_plan_allocation_grid_list"
            ).id,
    ...
    def action_create_invoice(self):
        """Validate the plan while ``qty_to_invoice`` still excludes this bill.

        ``purchase.order.action_create_invoice`` creates the draft bill before it
        calls ``action_view_invoice``.  Once that draft exists, its quantities are
        already included in ``qty_invoiced`` and can no longer be used to validate
        the current plan against the previously available quantity.
        """
        invoice_plan_id = self.env.context.get("invoice_plan_id")
        if invoice_plan_id:
            plan = self.env["purchase.invoice.plan"].browse(invoice_plan_id).exists()
            if plan and plan.allocation_method in ("manual", "sequential"):
                plan.purchase_id._validate_invoice_plan_allocations(
                    invoiceable_plan=plan
                )
