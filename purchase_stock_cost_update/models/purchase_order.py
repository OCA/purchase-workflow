...
class PurchaseOrderLine(models.Model):
    ...
    def _get_valued_in_moves(self):
        self.ensure_one()
        # `_get_po_line_moves` exists in newer Odoo versions, while in 14.0
        # purchase lines expose stock moves through `move_ids`.
        get_po_line_moves = getattr(self, "_get_po_line_moves", None)
        moves = get_po_line_moves() if get_po_line_moves else self.move_ids
        return moves.filtered(lambda m: m.state == "done" and m.product_qty != 0)
    ...
    def _apply_unit_price_difference(self):
        ...
        if hasattr(self.product_id, "invalidate_recordset"):
            self.product_id.invalidate_recordset(["value_svl", "quantity_svl"])
        else:
            self.product_id.invalidate_cache(fnames=["value_svl", "quantity_svl"])
        (
            price_unit,
            svl_qty,
            price_diff,
