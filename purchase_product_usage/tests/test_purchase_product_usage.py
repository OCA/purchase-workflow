...
class TestPurchaseOrder(common.SavepointCase):
    ...
    def setUpClass(cls):
        ...
        categ_manual = cls.env["product.category"].create(
            {"name": "Manual", "property_valuation": "manual_periodic"}
        )
        cls.product_1 = cls.product_obj.create(
            {"name": "Test product 1", "type": "consu", "categ_id": categ_manual.id}
        )
        cls.product_2 = cls.product_obj.create(
            {"name": "Test product 2", "type": "product", "categ_id": categ_manual.id}
        )
        cls.product_3 = cls.product_obj.create(
            {"name": "Test product 3", "type": "product", "categ_id": categ_manual.id}
        )
