...
class PurchaseOrderReceivedByCategory(models.Model):
    ...
    to_receive = fields.Float()


class PurchaseOrder(models.Model):
    _inherit = "purchase.order"

    purchased_quantity_total = fields.Float(
        string="Total Quantity", compute="_compute_purchased_quantity_total"
    )
    delivery_status = fields.Float(compute="_compute_delivery_status")
    received_by_category_ids = fields.One2many(
        "purchase.order.received.by.category",
        "order_id",
        string="Received by Category",
    ...
    def _compute_delivery_status(self):
        for order in self:
            received_quantity = sum(order.mapped("order_line.qty_received"))
            if order.purchased_quantity_total and received_quantity:
                order.delivery_status = (
                    received_quantity / order.purchased_quantity_total
                )
            else:
    ...
    def _compute_received_by_category_ids(self):
        for order in self:
            purchase_order_received_by_category_obj = self.env[
                "purchase.order.received.by.category"
            ]
            order_id = (
                order._origin.id if isinstance(order.id, models.NewId) else order.id
            )
            purchase_order_received_by_category_obj.search(
                [("order_id", "=", order_id)]
            ).unlink()
            order.received_by_category_ids = False
            received_by_category_ids = []
            for category in order.mapped("order_line.product_id.categ_id"):
                lines = order.order_line.filtered(
                    lambda l: l.product_id.categ_id == category
                )
                if lines:
                    values = {
                        "order_id": order_id,
                    ...
                    received_by_category_id = (
                        purchase_order_received_by_category_obj.create(values)
                    )
                    received_by_category_ids.append((4, received_by_category_id.id))
