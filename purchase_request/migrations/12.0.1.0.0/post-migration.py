...
def allocate_from_stock_move(env, ml, alloc_uom, ml_done=None):
    ...
    if ml_done is None:
        ml_done = []
    to_allocate_qty = ml.product_uom_id._compute_quantity(ml.qty_done, alloc_uom)
    for allocation in ml.filtered(
        lambda m: m.id not in ml_done
    ).move_id.purchase_request_allocation_ids:
        if (
            to_allocate_qty > 0.0
            and allocation.allocated_product_qty < allocation.requested_product_uom_qty
        ):
            allocated_qty = min(allocation.requested_product_uom_qty, to_allocate_qty)
...
def allocate_stockable(env):
    ...
    logger.info("Allocating purchase request for stockables " "and consumables")
    ...
    for (
        ...
        pending_qty = purchase_request_line.product_qty - purchase_request_line.qty_done
        if not pending_qty:
            continue
        if sm_id:
            ...
            if sm.state == "done":
                ml_done = allocate_from_stock_move(
                    env, sm.move_line_ids, alloc_uom, ml_done
                )
        else:
            # we allocated what is in the PR line
            req_qty = sm_uom._compute_quantity(req_qty, alloc_uom)
            create_allocation(
                env, purchase_order_line_id, purchase_request_line_id, False, req_qty
            )
...
def allocate_service(env):
    ...
    for (
        ...
        product_alloc_uom = purchase_request_line.product_uom_id.id or pol_product_uom
        ...
        alloc = create_service_allocation(
            env, purchase_order_line_id, purchase_request_line_id, product_alloc_qty
        )
