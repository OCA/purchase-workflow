...
def store_field_qty_in_progress(cr):
    if not openupgrade.column_exists(cr, "purchase_request_line", "qty_in_progress"):
        cr.execute(
            """
...
def store_field_qty_done(cr):
    if not openupgrade.column_exists(cr, "purchase_request_line", "qty_done"):
        cr.execute(
            """
...
def store_field_qty_cancelled(cr):
    if not openupgrade.column_exists(cr, "purchase_request_line", "qty_cancelled"):
        cr.execute(
            """
