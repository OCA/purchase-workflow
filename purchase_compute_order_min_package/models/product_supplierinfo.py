...
class ProductSupplierinfo(models.Model):
    ...
    def _convert_qty(self, quantity):
        ...
        # Limit the quantity by the minimum number of packages if applicable.
        if self.package_qty and self.min_nb_of_package > 0:
            min_quantity = self.min_nb_of_package * self.package_qty
            if quantity < min_quantity:
                # Did not meet the minimum number of packages requirement.
                quantity = 0
        # Limit the quantity by the maximum number of packages if applicable.
