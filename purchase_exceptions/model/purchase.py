...
class PurchaseException(models.Model):
    _name = 'purchase.exception'
    _description = "Purchase Exceptions"
    ...
    code = fields.Text(
        ...
        help="Python code executed to check if the exception apply or "
             ...
             "exception.",
        default="""
# Python code. Use failed = True to block the purchase order.
# You can use the following variables :
#  - self: ORM model of the record which is checked
#  - order or line: browse_record of the purchase order or purchase order line
#  - object: same as order or line, browse_record of the purchase order or
#    purchase order line
...
""")
    purchase_order_ids = fields.Many2many(
        'purchase.order',
        'purchase_order_exception_rel', 'exception_id', 'purchase_order_id',
...
class PurchaseOrder(models.Model):
    ...
    exception_ids = fields.Many2many(
        'purchase.exception',
        'purchase_order_exception_rel', 'purchase_order_id', 'exception_id',
        string='Exceptions')
    ...
    def _popup_exceptions(self):
        ...
        wizard_model = self.env['purchase.exception.confirm']
        ...
        view_id = model_data_model.get_object_reference(
            'purchase_exceptions', 'view_purchase_exception_confirm')[1]
        ...
        action = {
            ...
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'purchase.exception.confirm',
            'view_id': [view_id],
            'target': 'new',
            'nodestroy': True,
            'res_id': wizard.id,
    ...
    def _rule_eval(self, rule, obj_name, rec):
        ...
        try:
            ...
            raise except_orm(
                _('Error'),
                _('Error when evaluating the purchase exception '
                  'rule:\n %s \n(%s)') % (rule.name, e))
